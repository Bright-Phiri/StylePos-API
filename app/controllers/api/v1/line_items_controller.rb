...
class Api::V1::LineItemsController < ApplicationController
  def create
    ...
    line_item = order.line_items.create(line_item_params.merge(item_id: item.id, price: item.price + vat, vat:, total: item.price * params[:line_item][:quantity].to_i * (1 + (16.5 / 100))))

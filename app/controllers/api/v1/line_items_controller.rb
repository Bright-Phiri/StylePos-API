...
class Api::V1::LineItemsController < ApplicationController
  ...
  def create_or_update_line_item(order, item)
    ...
    vat = LineItem.calculate_vat_fraction(item.selling_price, line_item_params[:quantity].to_i)
    line_item = order.line_items.create(line_item_params.merge(item_id: item.id, price: item.selling_price, vat:, total: LineItem.calculate_total(item.selling_price, line_item_params[:quantity].to_i)))

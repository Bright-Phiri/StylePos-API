...
describe 'Line Items API', type: :request do
  ...
  describe 'PATCH /api/v1/orders/order_id/line_items/line_item_id/apply_discount' do
    context 'given valid discount value' do
      it 'apply discount to line item' do
        ...
        patch "/api/v1/line_items/#{line_item.id}/apply_discount", params: { line_item: { discount: 20 } }, headers: headers
    ...
    context "given invalid discount value" do
      it 'returns an error and unprocessable_entity status code' do
        ...
        order = Order.find(response_body['id'])
        line_item = order.line_items.last
        patch "/api/v1/line_items/#{line_item.id}/apply_discount", params: { line_item: { discount: order.total + 200 } }, headers: headers
        expect(response).to have_http_status(:unprocessable_entity)
        expect(JSON.parse(response.body)).to be_an(Array)
      end
    end

...
describe 'Items API', type: :request do
  ...
  describe 'GET /items' do
    ...
    before do
      FactoryBot.create(:item, category:, name: 'Tommy Hilfiger T-shirt', price: '24000', selling_price: '25000', reorder_level: 4, size: 'L', color: 'Black', barcode: 'GHGHG85944')
      FactoryBot.create(:item, category:, name: 'Converse Chuck Taylor sneakers', price: '65000', selling_price: '25000', size: 'L', reorder_level: 4, color: 'Black', barcode: 'GHGHG80044')
    end
  ...
  describe 'GET /api/v1/items/:id' do
    let!(:category) { FactoryBot.create(:category, name: 'Clothing', description: 'Clothing') }
    let!(:item) { FactoryBot.create(:item, category:, name: 'Example Item', color: 'Blue', size: 'M', price: 100.0, reorder_level: 4, selling_price: '25000', barcode: '1234567890') }
  ...
  describe 'POST /api/v1/items' do
    ...
    context 'given valid attributes' do
      it 'creates a new item' do
        post "/api/v1/categories/#{category.id}/items", params: { item: { category_id: category.id, name: 'Tommy Hilfiger T-shirt', reorder_level: 4, price: '24000', selling_price: 2500, size: 'L', color: 'Black'} }, headers: headers
    ...
    context 'given invalid attributes' do
      it 'returns errors and unprocessable_entity status code' do
        post "/api/v1/categories/#{category.id}/items", params: { item: { category_id: category.id, name: '', color: 'Blue', size: 'M', reorder_level: 4, price: 'fhfh', selling_price: '25000' } }, headers: headers
  ...
  describe 'PUT /api/v1/items/:id' do
    let!(:category) { FactoryBot.create(:category, name: 'Clothing', description: 'Clothing') }
    let!(:item) { FactoryBot.create(:item, category:, name: 'Tommy Hilfiger T-shirt', price: '24000', selling_price: '25000', reorder_level: 4, size: 'L', color: 'Black', barcode: 'GHGHG85944') }

    context 'given valid attributes' do
      it 'updates an item' do
        new_attributes = { name: 'Tommy Hilfiger T-shirt', price: '24000', selling_price: '25000', reorder_level: 4, size: 'M', color: 'Red' }
    ...
    context 'given invalid attributes' do
      it 'returns an error and unprocessable_entity status code' do
        put "/api/v1/items/#{item.id}", params: { item: { name: 'Tommy Hilfiger T-shirt', price: '', selling_price: '25000', reorder_level: 4, size: 'M', color: '' } }, headers: headers
        expect(response).to have_http_status(:unprocessable_entity)
        expect(JSON.parse(response.body)).to be_an(Array)
        category.reload
